...
::std::vector<Command*> get_economy_commands(Database* db) {
    ...
    if (cmds.empty()) {
        ...
        auto core = get_economy_core_commands(db);  // Now includes /money parent
        
        // --- Re-inject text-only aliases for money commands ---
        {
            auto bal = economy::create_balance_command(db); bal->is_slash_command = false; core.push_back(bal);
            
            auto bnk = economy::create_bank_command(db); bnk->is_slash_command = false;
            bnk->aliases.push_back("dep");
            core.push_back(bnk);
            
            auto wth = economy::create_withdraw_command(db); wth->is_slash_command = false;
            wth->aliases.push_back("w"); wth->aliases.push_back("with");
            core.push_back(wth);
            
            auto pay = economy::create_pay_command(db); pay->is_slash_command = false; core.push_back(pay);
            auto pre = economy::create_prestige_command(db); pre->is_slash_command = false; core.push_back(pre);
            auto reb = economy::create_rebirth_command(db); reb->is_slash_command = false; core.push_back(reb);
            
            for (auto cmd : ::commands::get_money_commands(db)) {
                if (cmd) { cmd->is_slash_command = false; core.push_back(cmd); }
            }
            for (auto cmd : ::commands::get_rob_commands(db)) {
                if (cmd) { cmd->is_slash_command = false; core.push_back(cmd); }
            }
        }
