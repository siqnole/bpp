...
inline Command* get_massban_command(bronx::db::Database* db) {
    ...
    cmd = new Command(
        "massban", "ban multiple users at once", "moderation",
        {"mban"}, true,
        // text handler
        [db](dpp::cluster& bot, const dpp::message_create_t& event, const std::vector<std::string>& args) {
            ...
            if (args.empty()) {
                bronx::send_message(bot, event, bronx::error("usage: `massban <user1> [user2...] [reason: reason text]`"));
                return;
            }
            ...
            if (targets.empty()) {
                bronx::send_message(bot, event, bronx::error("no valid users to ban provided"));
                return;
            }
            ...
            auto embed = bronx::create_embed("mass banning **" + std::to_string(targets.size()) + "** users...", get_action_color("ban"));
        ...
        [db](dpp::cluster& bot, const dpp::slashcommand_t& event) {
            ...
            if (users_opt.index() == 0) {
                dpp::interaction_modal_response modal("mod_massban_modal", "Mass Ban Users");
                modal.add_component(
            ...
            std::string reason = "mass ban via interaction";
        ...
        {
            dpp::command_option(dpp::co_string, "users", "list of users to ban (IDs or mentions)", true),
