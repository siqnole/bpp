...
inline Command* get_masskick_command(bronx::db::Database* db) {
    ...
    cmd = new Command(
        "masskick", "kick multiple users at once", "moderation",
        {"mkick"}, true,
        // text handler
        [db](dpp::cluster& bot, const dpp::message_create_t& event, const std::vector<std::string>& args) {
            ...
            if (args.empty()) {
                bronx::send_message(bot, event, bronx::error("usage: `masskick <user1> [user2...] [reason: reason text]`"));
                return;
            }
            ...
            if (targets.empty()) {
                bronx::send_message(bot, event, bronx::error("no valid users to kick provided"));
                return;
            }
            ...
            auto embed = bronx::create_embed("mass kicking **" + std::to_string(targets.size()) + "** users...", get_action_color("kick"));
            bot.message_create(dpp::message(event.msg.channel_id, embed), [targets, &bot, db, guild_id, mod_id, reason, config, event](const dpp::confirmation_callback_t& cb) {
        ...
        [db](dpp::cluster& bot, const dpp::slashcommand_t& event) {
            ...
            if (users_opt.index() == 0) {
                dpp::interaction_modal_response modal("mod_masskick_modal", "Mass Kick Users");
                modal.add_component(
            ...
            std::string reason = "mass kick via interaction";
        ...
        {
            dpp::command_option(dpp::co_string, "users", "list of users to kick (IDs or mentions)", true),
