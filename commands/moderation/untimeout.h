...
inline Command* get_untimeout_command(bronx::db::Database* db) {
    static Command* cmd = nullptr;
    if (cmd) return cmd;

    cmd = new Command("untimeout", "remove a timeout from a user", "moderation", {"removetimeout"}, true,
        // ── text handler ──
        [db](dpp::cluster& bot, const dpp::message_create_t& event, const std::vector<std::string>& args) {
            ...
            if (args.empty()) {
                bronx::send_message(bot, event, bronx::error("usage: untimeout <@user> [reason]"));
                return;
            }
            ...
            // remove native timeout (set to 0 / epoch)
            bot.guild_member_timeout(guild_id, user_id, 0,
                [&bot, db, guild_id, user_id, mod_id, reason, event](const dpp::confirmation_callback_t& cb) {
                    if (cb.is_error()) {
                        bronx::send_message(bot, event, bronx::error("failed to remove timeout: " + cb.get_error().message));
                        return;
                    }

                    // pardon active timeout infractions
                    ...
                    for (auto& inf : infractions) {
                        if (inf.type == "timeout" && inf.active && !inf.pardoned) {
                    ...
                    bronx::send_message(bot, event, bronx::success(
                        "removed timeout from <@" + std::to_string(user_id) + ">"
        ...
        [db](dpp::cluster& bot, const dpp::slashcommand_t& event) {
            ...
            bot.guild_member_timeout(guild_id, user_id, 0,
                [&bot, db, guild_id, user_id, mod_id, reason, event](const dpp::confirmation_callback_t& cb) {
                    if (cb.is_error()) {
                        event.reply(dpp::message().add_embed(bronx::error("failed to remove timeout: " + cb.get_error().message)).set_flags(dpp::m_ephemeral));
                        return;
                    }
                    ...
                    event.reply(dpp::message().add_embed(bronx::success(
                        "removed timeout from <@" + std::to_string(static_cast<uint64_t>(user_id)) + ">"
                        + (pardoned_count > 0 ? " — pardoned **" + std::to_string(pardoned_count) + "** infraction(s)" : "")
                    )));
                });
        },
        // ── slash options ──
        {
            dpp::command_option(dpp::co_user, "user", "the user to remove timeout from", true),
            dpp::command_option(dpp::co_string, "reason", "reason for removing the timeout", false)
        }
    );
