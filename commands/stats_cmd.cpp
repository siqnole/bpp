...
void attach_stats_buttons(dpp::message& msg, const std::string& category, int days, uint64_t user_id,
                         const std::string& gtype, const std::string& top_filter) {
...
int parse_days(const std::vector<std::string>& args, size_t idx) {
    if (idx < args.size()) {
...
dpp::command_option range_option() {
    return dpp::command_option(dpp::co_string, "range", "time range to view")
        ...
        .add_choice(dpp::command_option_choice("last 14 days", std::string("14d")))
...
void handle_stats_summary(dpp::cluster& bot, bronx::db::Database* db,
                         ...
                         uint64_t user_id, const std::string& gtype) {
