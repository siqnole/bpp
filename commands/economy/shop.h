...
#include "../owner.h" // for is_owner helper
...
// ============================================================================
// SHOP PAGINATION CONSTANTS
// ============================================================================
constexpr int ITEMS_PER_PAGE = 5;  // Number of items per page in shop display

// shopping catalog is stored in the database; use the shared type
using bronx::db::ShopItem;

// Parse required prestige from item metadata JSON (returns 0 if none)
static int get_required_prestige(const std::string& metadata) {
...
static std::string prestige_indicator(int prestige) {
    if (prestige <= 0) return "";
    return " " + bronx::EMOJI_STAR + "P" + std::to_string(prestige);
}

// Convert item id to a friendly display name (fall back to id if unknown)
...
// helper to fetch current shop items from the database
static ::std::vector<ShopItem> load_shop_items(bronx::db::Database* db) {
    return db->get_shop_items();
}

// Attempt to resolve a user-provided token to a shop item.  We try exact
// match on id first (case-insensitive), then look for substrings in the id or
// name. This lets users type "wood" or "common" instead of the full
// `rod_wood`/`bait_common` id.
static std::optional<ShopItem> find_shop_item(Database* db, const std::string& token) {
    std::string lower = token;
    std::transform(lower.begin(), lower.end(), lower.begin(), ::tolower);
    auto items = load_shop_items(db);
    ...
    if (lower == "pi") {
        for (const auto& it : items) {
            if (it.item_id == "bait_pi") return it;
        }
    }
    ...
    // 6. item_id contains token (prefer shorter matches)
    std::vector<std::pair<ShopItem, size_t>> id_matches;
    for (const auto& it : items) {
        std::string id = it.item_id;
        std::transform(id.begin(), id.end(), id.begin(), ::tolower);
        auto pos = id.find(lower);
        if (pos != std::string::npos) {
            id_matches.push_back({it, id.length()});
        }
    }
    if (!id_matches.empty()) {
...
// (UTC).  Titles rotate once per week; this is the beginning of the next week.
...
static std::string normalize_category(std::string cat) {
    std::transform(cat.begin(), cat.end(), cat.begin(), ::tolower);
    
    // Handle common aliases
    if (cat == "fishing bait" || cat == "fishingbait") {
        return "bait";
    }
    if (cat == "fishing rod" || cat == "fishingrod" || cat == "rods") {
        return "rod";
    }
    if (cat == "baits") {
        return "bait";
    }
    if (cat == "title" || cat == "titles" || cat == "cosmetic" || cat == "cosmetics") {
        return "title";
    }
    if (cat == "pickaxes" || cat == "pick") {
        return "pickaxe";
    }
    if (cat == "minecarts" || cat == "cart" || cat == "carts") {
        return "minecart";
    }
    if (cat == "bags" || cat == "mining bag" || cat == "mining bags") {
        return "bag";
    }
