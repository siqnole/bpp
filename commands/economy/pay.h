...
// ─────────────────────────────────────────────────────────────────────────────
inline std::string do_pay_checks(Database *db, uint64_t sender_id,
                                 uint64_t recipient_id, int64_t amount,
  ...
  // set_daily_stat here instead.  Adjust the call site to match your API.
  {
    ...
    int64_t last_pay_ts = ::commands::daily_challenges::get_daily_stat(
        db, sender_id, cooldown_key);
    int64_t now = static_cast<int64_t>(std::time(nullptr));
    if (last_pay_ts > 0 && (now - last_pay_ts) < PAY_COOLDOWN_SECONDS) {
      int64_t wait = PAY_COOLDOWN_SECONDS - (now - last_pay_ts);
      return "you're paying that user too fast — wait " + std::to_string(wait) +
             "s";
    }
  }
  ...
  int64_t already_received = ::commands::daily_challenges::get_daily_stat(
      db, recipient_id, "coins_received_today");
  int64_t recipient_baseline = recipient_current - already_received;
  if (recipient_baseline < 0)
    recipient_baseline = 0;
  ...
  if (recipient_baseline > 0) {
    ...
    if (amount > BOOTSTRAP_CAP) {
      return "that user has no prior balance — max first-day payment is $" +
             format_number(BOOTSTRAP_CAP);
    }
  }
  ...
  if (RECIPIENT_DAILY_RECEIVE_MULTIPLIER > 0 && recipient_baseline > 0) {
    ...
    if (would_receive > daily_cap) {
      ...
      return "that would exceed the daily receive limit — max you can send "
             "them today is $" +
...
// ─────────────────────────────────────────────────────────────────────────────
inline void record_pay_stats(Database *db, uint64_t sender_id,
                             uint64_t recipient_id, int64_t amount) {
  // Track how much the recipient has received today (used by Layer 1 & 2).
  ::commands::daily_challenges::track_daily_stat(
      db, recipient_id, "coins_received_today", amount);
  ...
  int64_t now = static_cast<int64_t>(std::time(nullptr));
  std::string cooldown_key = "last_pay_to_" + std::to_string(recipient_id);
  ::commands::daily_challenges::set_daily_stat(db, sender_id, cooldown_key,
                                               now);

  // Existing daily challenge stat — unchanged.
  ::commands::daily_challenges::track_daily_stat(db, sender_id,
                                                 "coins_paid_today", amount);
}

inline Command *create_pay_command(Database *db) {
  static Command *pay = new Command(
      "pay", "transfer money to another user", "economy", {"give"}, true,
      [db](dpp::cluster &bot, const dpp::message_create_t &event,
           const ::std::vector<::std::string> &args) {
        if (event.msg.mentions.empty()) {
          bronx::send_message(bot, event,
                              bronx::error("please mention a user to pay"));
          return;
        }

        if (args.size() < 2) {
          bronx::send_message(bot, event,
                              bronx::error("please specify an amount"));
          return;
        }
        ...
        if (recipient_id == event.msg.author.id) {
          bronx::send_message(bot, event,
                              bronx::error("you can't pay yourself"));
          return;
        }
        ...
        int64_t sender_wallet =
            server_mode ? get_server_wallet(db, *guild_id, event.msg.author.id)
                        : user->wallet;
        ...
        if (amount <= 0) {
          bronx::send_message(bot, event,
                              bronx::error("amount must be positive"));
          return;
        }

        if (amount > sender_wallet) {
          bronx::send_message(bot, event,
                              bronx::error("you don't have that much"));
          return;
        }

        // Block transfers between prestiged and non-prestiged users (global
        // only)
        if (!server_mode) {
          ...
          if ((sender_prestige == 0) != (recipient_prestige == 0)) {
            bronx::send_message(
                bot, event,
                bronx::error("you can't transfer money between prestiged and "
                             "non-prestiged players"));
            return;
          }
        }
        ...
        {
          std::string err = do_pay_checks(db, event.msg.author.id, recipient_id,
                                          amount, guild_id);
          if (!err.empty()) {
        ...
        auto sender_result =
            update_wallet_unified(db, event.msg.author.id, guild_id, -amount);
        if (!sender_result) {
          bronx::send_message(bot, event,
                              bronx::error("failed to transfer money"));
          return;
        }
        auto recipient_result =
            update_wallet_unified(db, recipient_id, guild_id, received);
        if (recipient_result) {
          ...
          int64_t sender_balance =
              get_wallet_unified(db, event.msg.author.id, guild_id);
          int64_t recipient_balance =
              get_wallet_unified(db, recipient_id, guild_id);
          std::string tax_str =
              tax > 0 ? (" (tax: $" + format_number(tax) + ")") : "";
          std::string sender_log = "paid <@" + std::to_string(recipient_id) +
                                   "> $" + format_number(amount) + tax_str;
          std::string recipient_log =
              "received $" + format_number(received) + " from <@" +
              std::to_string(event.msg.author.id) + ">" +
              (tax > 0 ? " (after " + std::to_string((int)tax_rate) + "% tax)"
                       : "");
          bronx::db::history_operations::log_payment(
              db, event.msg.author.id, sender_log, -amount, sender_balance);
          bronx::db::history_operations::log_payment(
              db, recipient_id, recipient_log, received, recipient_balance);
          ...
          if (guild_id) {
            dpp::embed log_emb =
                bronx::info("Economy Transaction: Payment")
                    .set_color(0x00FF00)
                    .add_field("Sender",
                               "<@" + std::to_string(event.msg.author.id) + ">",
                               true)
                    .add_field("Recipient",
                               "<@" + std::to_string(recipient_id) + ">", true)
                    .add_field("Amount", "$" + format_number(amount), true);
            if (tax > 0)
              log_emb.add_field("Tax", "$" + format_number(tax), true);
            log_emb.set_timestamp(time(0));
            bronx::logger::ServerLogger::get().log_embed(
                *guild_id, bronx::logger::LOG_TYPE_ECONOMY, log_emb);
          }

          std::string tax_display = "";
          if (tax > 0) {
            tax_display = "\n💸 " + std::to_string((int)tax_rate) + "% tax: $" +
                          format_number(tax);
            tax_display +=
                "\n🗑️ $" + format_number(tax_destroyed) + " destroyed";
            if (tax_to_guild > 0) {
              tax_display += "\n🏦 $" + format_number(tax_to_guild) +
                             " added to server giveaway balance";
            }
          }
          auto embed = bronx::success(
              "transferred $" + format_number(received) + " to " +
              event.msg.mentions.begin()->first.format_username() +
              tax_display);
      ...
      [db](dpp::cluster &bot, const dpp::slashcommand_t &event) {
        ...
        if (!std::holds_alternative<dpp::snowflake>(user_param)) {
          event.reply(
              dpp::message().add_embed(bronx::error("please mention a user")));
          return;
        }
        ...
        if (std::holds_alternative<std::string>(amount_param)) {
          ...
          event.reply(dpp::message().add_embed(
              bronx::error("please provide an amount")));
          return;
        }

        if (recipient_id == event.command.get_issuing_user().id) {
          event.reply(
              dpp::message().add_embed(bronx::error("you can't pay yourself")));
          return;
        }
        ...
        if (recipient.is_bot()) {
          event.reply(
              dpp::message().add_embed(bronx::error("you can't pay bots")));
          return;
        }
        ...
        if (event.command.guild_id)
          guild_id = static_cast<uint64_t>(event.command.guild_id);
        bool server_mode = guild_id && is_server_economy(db, *guild_id);

        int64_t sender_wallet =
            server_mode ? get_server_wallet(db, *guild_id,
        ...
        if (!server_mode) {
          int sender_prestige =
              db->get_prestige(event.command.get_issuing_user().id);
          int recipient_prestige = db->get_prestige(recipient_id);
          if ((sender_prestige == 0) != (recipient_prestige == 0)) {
            event.reply(dpp::message().add_embed(
                bronx::error("you can't transfer money between prestiged and "
        ...
        {
          std::string err =
              do_pay_checks(db, sender_id, recipient_id, amount, guild_id);
          if (!err.empty()) {
        ...
        // Perform transfer using unified operations
        auto sender_result =
            update_wallet_unified(db, sender_id, guild_id, -amount);
        if (!sender_result) {
          event.reply(dpp::message().add_embed(
              bronx::error("failed to transfer money")));
          return;
        ...
        if (recipient_result) {
          ...
          std::string recipient_log =
              ...
              (tax > 0 ? " (after " + std::to_string((int)tax_rate) + "% tax)"
                       : "");
          bronx::db::history_operations::log_payment(db, sender_id, sender_log,
          ...
          if (guild_id) {
            dpp::embed log_emb =
                bronx::info("Economy Transaction: Payment")
                    .set_color(0x00FF00)
                    .add_field("Sender", "<@" + std::to_string(sender_id) + ">",
                               true)
                    .add_field("Recipient",
                               "<@" + std::to_string(recipient_id) + ">", true)
                    .add_field("Amount", "$" + format_number(amount), true);
          ...
          auto embed = bronx::success(
              "transferred $" + format_number(received) + " to " +
              recipient.format_username() + tax_display);
      ...
      {dpp::command_option(dpp::co_user, "user", "user to pay", true),
       dpp::command_option(dpp::co_string, "amount",
                           "amount to pay (supports all, half, 50%, 1k, etc)",
                           true)});
  return pay;
}
