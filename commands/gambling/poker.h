...
// ============================================================================
// TEXAS HOLD'EM POKER — Full multiplayer game
// ============================================================================
// 2–6 players. Standard Texas Hold'em rules:
//   Lobby → Preflop → Flop → Turn → River → Showdown
// Blinds: SB = min_bet/2, BB = min_bet.
// Betting: Check/Call, Raise 2x, All-In, Fold.
// Hole cards sent once via DM at deal time.
// ============================================================================
...
static std::vector<PokerCard> poker_make_deck() {
...
static void poker_shuffle(std::vector<PokerCard>& d) {
    std::mt19937 g(std::random_device{}());
    std::shuffle(d.begin(), d.end(), g);
}

// ── Hand evaluation ───────────────────────────────────────────────────────────
// Returns std::vector<int> {category, tiebreaker...}
// Lexicographic comparison gives better hands the higher score.
// Categories: 0=high card, 1=pair, 2=two pair, 3=three of a kind,
//             4=straight, 5=flush, 6=full house, 7=four of a kind, 8=str flush

using HandScore = std::vector<int>;

static HandScore eval_5(std::vector<int> ranks, bool flush) {
    ...
    // Check straight (5 distinct ranks, span of 4 OR A-2-3-4-5 wheel)
...
// Best hand from up to 7 cards (hole + community)
static HandScore best_hand(const std::vector<PokerCard>& cards) {
    HandScore best;
...
struct PokerPlayer {
    uint64_t    user_id;
    std::string display_name;           // non-empty for CPU bots
    bool        is_cpu      = false;
    std::vector<PokerCard> hole;
    int64_t  stack          = 0;        // chips remaining
    int64_t  bet_street     = 0;        // amount bet this street
    bool     folded         = false;
    bool     all_in         = false;
};
...
struct PokerGame {
    ...
    uint64_t guild_id;
    uint64_t host_id;
    int64_t  min_bet   = 1000;  // big blind = min_bet; buy-in = 50 * min_bet
    int64_t  pot       = 0;
    int64_t  cur_bet   = 0;     // highest bet on this street
    ...
    int dealer_idx     = 0;
    std::deque<int>    to_act;    // player indices who still need to act

    dpp::snowflake msg_id = 0;
    bool started = false;
};

static std::map<uint64_t, PokerGame> g_poker_games; // keyed by channel_id
static std::mutex g_poker_mutex;

// Player mention-or-name
static std::string pname(const PokerPlayer& p) {
    return p.is_cpu ? p.display_name : "<@" + std::to_string(p.user_id) + ">";
}

// ── Helpers ───────────────────────────────────────────────────────────────────
static std::string cards_str(const std::vector<PokerCard>& cv) {
    std::string s;
    for (auto& c : cv) s += c.display() + " ";
    return s.empty() ? "*(none)*" : s;
}

static int active_count(const PokerGame& g) {
    int n = 0;
    for (auto& p : g.players) if (!p.folded) n++;
    return n;
}

// ── Build public game message ──────────────────────────────────────────────────
static dpp::message build_poker_msg(const PokerGame& g) {
    ...
    if (g.phase == PokerPhase::Lobby && !g.started) {
        ...
        // Show action buttons for the current bettor
...
static void send_hole_cards(dpp::cluster& bot, const PokerGame& g) {
...
static void cpu_take_action(dpp::cluster& bot, Database* db, uint64_t chan_id); // fwd

static void notify_current_player(dpp::cluster& bot, Database* db, const PokerGame& g, uint64_t chan_id) {
...
// ── Build to_act queue for a street ───────────────────────────────────────────
static void build_to_act_postflop(PokerGame& g) {
    g.to_act.clear();
    g.cur_bet = 0;
...
// Returns false if no more cards can/should be dealt (showdown triggered)
static void advance_phase(dpp::cluster& bot, Database* db, uint64_t chan_id);

static void do_showdown(dpp::cluster& bot, Database* db, uint64_t chan_id) {
    ...
    // Winners = all with best hand (tie)
    ...
    for (int w : winners) {
        ...
        if (!g.players[w].is_cpu) {
            ...
            track_gambling_result(bot, db, chan_id, g.players[w].user_id, true, payout);
        }
    }
    ...
    for (auto& p : g.players) {
        ...
        if (!p.folded) {
            ...
            if (!won)
                track_gambling_result(bot, db, chan_id, p.user_id, false, -g.min_bet);
        }
    }
...
// Winner by fold (only 1 active player left)
static void award_fold_win(dpp::cluster& bot, Database* db, uint64_t chan_id) {
    std::unique_lock<std::mutex> lk(g_poker_mutex);
    auto it = g_poker_games.find(chan_id);
    if (it == g_poker_games.end()) return;
    PokerGame& g = it->second;
    ...
    if (!g.players[winner].is_cpu) {
        ...
        track_gambling_result(bot, db, chan_id, g.players[winner].user_id, true, g.pot);
    }
...
static void advance_phase(dpp::cluster& bot, Database* db, uint64_t chan_id) {
    {
        ...
        // Only 1 player left?
        if (active_count(g) <= 1) lk.~lock_guard(); // release before award
        else {
            switch (g.phase) {
                ...
                case PokerPhase::River:
                    lk.~lock_guard(); // release before showdown
                    do_showdown(bot, db, chan_id);
                    return;
                default: break;
            }
            // If to_act is empty (e.g., all remaining players are all-in), skip to showdown
    ...
    // Check fold win or update message
    {
        std::lock_guard<std::mutex> lk(g_poker_mutex);
        auto it = g_poker_games.find(chan_id);
        if (it == g_poker_games.end()) return;
...
// ── CPU logic ──────────────────────────────────────────────────────────────────
static float cpu_hand_strength(const std::vector<PokerCard>& hole,
...
static std::string cpu_decide(float strength, int64_t to_call, int64_t stack) {
...
static void cpu_take_action(dpp::cluster& bot, Database* db, uint64_t chan_id) {
...
static void start_poker_game(dpp::cluster& bot, Database* db, uint64_t chan_id) {
    std::unique_lock<std::mutex> lk(g_poker_mutex);
    auto it = g_poker_games.find(chan_id);
    if (it == g_poker_games.end() || it->second.started) return;
    PokerGame& g = it->second;

    // Need at least 2 total players (CPUs count) and at least 1 human
    int human_count = 0;
    ...
    // Build preflop to_act queue: UTG → ... → SB → BB
    g.to_act.clear();
    for (int i = 0; i < n; i++) {
        int p = (utg + i) % n;
...
// ── Process an action ─────────────────────────────────────────────────────────
static void process_action(dpp::cluster& bot, Database* db, uint64_t chan_id,
...

    {
        ...
        int cur_idx = g.to_act.front();
        // Reject if it's a CPU's turn (shouldn't happen normally since buttons are hidden)
        ...
        if (g.players[cur_idx].user_id != actor) {
            // Let them see "My Cards" even when not their turn
            if (action == "mycards") {
        ...
        // It IS this player's turn — process action
        auto& p = g.players[cur_idx];
        ...
        if (action == "fold") {
            ...
            int64_t to_pay   = std::min(p.stack, raise_to - p.bet_street);
            if (to_pay <= 0 || to_pay >= p.stack) {
                // Not enough to raise — treat as all-in
                to_pay = p.stack;
            }
            p.stack      -= to_pay;
            p.bet_street += to_pay;
            ...
            if (p.stack == 0) p.all_in = true;
            g.cur_bet = std::max(g.cur_bet, p.bet_street);
            // Re-open action: everyone except raiser
            g.to_act.pop_front();
            std::deque<int> new_q;
            int n = (int)g.players.size();
            for (int i = 1; i <= n; i++) {
        ...
        // Check if phase is done (to_act empty) or only 1 player left
        if (active_count(g) <= 1 || g.to_act.empty()) needs_advance = true;

        // Guard: reject button clicks when it's a CPU's turn
        (void)0; // handled above via is_cpu check at entry
    ...
    static Command* cmd = new Command(
        ...
        [db](dpp::cluster& bot, const dpp::message_create_t& event, const std::vector<std::string>& args) {
            ...
            if (sub == "start") {
                ...
                // Add CPU players (no wallet deduction; use fake IDs 1–5)
                for (int i = 0; i < cpu_count; i++) {
                    PokerPlayer cpu;
                    cpu.is_cpu       = true;
                    cpu.user_id      = (uint64_t)(i + 1); // fake, won't be @-mentioned
                    cpu.display_name = "🤖 CPU " + std::to_string(i + 1);
                    cpu.stack        = buy_in; // same chips, no real wallet
...
inline void register_poker_interactions(dpp::cluster& bot, Database* db) {
    // Join button
    ...
    // Gameplay action buttons (checkcall / raise / allin / fold / mycards)
