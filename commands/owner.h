...
static uint64_t parse_snowflake(const std::string &s) {
    uint64_t id = 0;
    for (char c : s) {
...
static bool parse_scope_args(const std::vector<std::string> &args,
                             size_t idx,
...
inline void register_owner_interactions(dpp::cluster& bot, bronx::db::Database* db, ::CommandHandler* handler) {
